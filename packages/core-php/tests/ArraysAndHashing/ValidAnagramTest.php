...
final class ValidAnagramTest extends TestCase
{
    ...
    {
        $solve = ValidAnagram::solutions()[$approach];

        $this->assertSame($expected, $solve($input['s'], $input['t']));
    }
    ...
    {
        $results = array_map(
            static fn (callable $solve): mixed => $solve($input['s'], $input['t']),
            ValidAnagram::solutions(),
        );
    ...
    public static function contractCases(): iterable
    {
        ...
        foreach (array_keys(ValidAnagram::solutions()) as $approach) {
            foreach ($contracts->cases(ValidAnagram::SLUG) as $case) {
    ...
    {
        foreach ((new ContractRepository())->cases(ValidAnagram::SLUG) as $case) {
