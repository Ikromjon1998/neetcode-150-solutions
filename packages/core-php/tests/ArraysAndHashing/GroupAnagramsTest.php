 ...
 */
final class GroupAnagramsTest extends TestCase
{
    #[Test]
    #[DataProvider('contractCases')]
    public function it_solves_every_contract_case(string $approach, array $input, mixed $expected): void
    {
        $solve = GroupAnagrams::solutions()[$approach];

        $this->assertSame($expected, $solve($input['strs']));
    }
    ...
    {
        $results = array_map(
            static fn (callable $solve): mixed => $solve($input['strs']),
            GroupAnagrams::solutions(),
        );
    ...
    public static function contractCases(): iterable
    {
        ...
        foreach (array_keys(GroupAnagrams::solutions()) as $approach) {
            foreach ($contracts->cases(GroupAnagrams::SLUG) as $case) {
    ...
    {
        foreach ((new ContractRepository())->cases(GroupAnagrams::SLUG) as $case) {
