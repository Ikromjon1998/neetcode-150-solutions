...
packages/contracts/problems/0242-valid-anagram.json
...
def test_solves_every_contract_case(approach: str, case: dict) -> None:
    ...
    assert solve(case["input"]["s"], case["input"]["t"]) == case["expected"]
...
def test_all_approaches_agree(case: dict) -> None:
    ...
    results = [
        get_solution(SLUG, approach=approach)(case["input"]["s"], case["input"]["t"])
