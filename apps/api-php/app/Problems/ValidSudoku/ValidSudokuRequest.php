...
/**
 * Request validation for `POST /problems/valid-sudoku`.
 *
 ...
 */
final class ValidSudokuRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            'board' => ['present', 'array'],
            'board.*' => ['present', 'array'],
            'board.*.*' => ['present', $this->strictType('is_string', 'board.*.*')],
        ];
    }
    ...
    {
        $unexpected = array_diff(array_keys($this->json()->all()), ['board']);
