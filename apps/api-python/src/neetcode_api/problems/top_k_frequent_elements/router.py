...
from neetcode_api.problems.top_k_frequent_elements.schemas import TopKFrequentElementsRequest
from neetcode_api.problems.top_k_frequent_elements.service import SLUG, TopKFrequentElementsService
...
@router.post(
    f"/{SLUG}",
    summary="Solve Top K Frequent Elements",
...
async def solve(
    payload: TopKFrequentElementsRequest,
    approach: Annotated[str, Depends(approach_provider(SLUG))],
    service: Annotated[
        TopKFrequentElementsService, Depends(TopKFrequentElementsService)
    ],
) -> SolveResponse[list[int]]:
    result, elapsed = service.solve(payload.nums, payload.k, approach)
